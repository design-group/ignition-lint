...
def main():
    ...
    parser.add_argument(
        "--files",
        default="**/view.json",
        help="Comma-separated list of files or glob patterns to lint (manual use)",
    )
    parser.add_argument(
        "--component-style",
        default="PascalCase",
    ...
    # Debug: Print arguments to confirm what’s being passed
    print(f"Args received: {args}")

    # Use positional files if provided (pre-commit), else fall back to --files
    files_to_lint = args.files if args.files else args.files_option.split(",")
    ...
    for file_pattern in files_to_lint:
        if re.search(r"[\*\?\[\]]", file_pattern):  # Glob pattern
