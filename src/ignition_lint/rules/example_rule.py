...
class ExampleNameLengthRule(LintingRule):
    ...
    def visit_component(self, component: ViewNode):
        ...
        if len(component_name) < self.min_length:
            self.errors.append(
                f"{component.path}: Component name '{component_name}' is too short "
...
class ExampleBindingCountRule(LintingRule):
    ...
    def visit_component(self, component: ViewNode):
        ...
        self.component_bindings[component.path] = 0
    
    def visit_expression_binding(self, binding: ViewNode):
        """Count expression bindings."""
        self._count_binding(binding)
    
    def visit_property_binding(self, binding: ViewNode):
    ...
    def _count_binding(self, binding: ViewNode):
...
# Manual registration (alternative to decorator) - commented out to avoid duplicate
# register_rule(ExampleBindingCountRule, "ExampleBindingCountRule")