...
class TestExampleMixedSeverityRule(BaseRuleTest):
	...
	def test_warnings_for_style_issues(self):
		...
		components = [
			{"name": "tempButton", "type": "button"},  # WARNING: temp naming
			{"name": "x", "type": "label"},           # WARNING: very short name  
			{"name": "MyComponent", "type": "container"}  # WARNING: no type suffix
		]
		...
		self.assert_rule_summary(mock_view, rule_config, "ExampleMixedSeverityRule",
			expected_warnings=3, expected_errors=0)

	def test_errors_for_functional_issues(self):
		"""Test that functional issues produce errors."""  
		# Create a view with functional issues
		components = [
			{"name": "UnsafeComponent", "type": "container"},     # ERROR: unsafe name
			{"name": "DebugProdPanel", "type": "panel"}           # ERROR: conflicting indicators
		]
		mock_view_content = create_mock_view(components)
		from fixtures.test_helpers import create_temp_view_file
		mock_view = create_temp_view_file(mock_view_content)
		...
		self.assert_rule_summary(mock_view, rule_config, "ExampleMixedSeverityRule",
			expected_warnings=1, expected_errors=2)  # UnsafeComponent gets suffix warning
	...
	def test_mixed_warnings_and_errors(self):
		...
		components = [
			{"name": "tempButton", "type": "button"},         # WARNING: temp naming
			{"name": "AdminPanel", "type": "panel"},          # ERROR: potentially unsafe
			{"name": "x", "type": "label"},                   # WARNING: short name
			{"name": "TestLiveComponent", "type": "container"} # ERROR: conflicting indicators
		]
		mock_view_content = create_mock_view(components)
		from fixtures.test_helpers import create_temp_view_file
		mock_view = create_temp_view_file(mock_view_content)
		...
		self.assert_rule_summary(mock_view, rule_config, "ExampleMixedSeverityRule",
			expected_warnings=4, expected_errors=2)
	...
	def test_clean_components_pass(self):
		...
		components = [
			...
			{"name": "UserNameLabel", "type": "label"},
			{"name": "DataPanel", "type": "panel"},
			{"name": "MainContainer", "type": "container"}
		]
		mock_view_content = create_mock_view(components)
		from fixtures.test_helpers import create_temp_view_file
		mock_view = create_temp_view_file(mock_view_content)
	...
	def test_warning_patterns(self):
		...
		self.assert_rule_warnings(mock_view, rule_config, "ExampleMixedSeverityRule",
	...
	def test_error_patterns(self):
		...
		# Check error content  
		self.assert_rule_errors(mock_view, rule_config, "ExampleMixedSeverityRule",
	...
	def test_exception_cases(self):
		...
		components = [
			{"name": "ok", "type": "button"},  # Should not warn - common short name
			{"name": "id", "type": "label"},   # Should not warn - common short name  
			{"name": "go", "type": "button"}   # Should not warn - common short name
		]
		mock_view_content = create_mock_view(components)
		from fixtures.test_helpers import create_temp_view_file
		mock_view = create_temp_view_file(mock_view_content)
