...
class JsonLinter:
    ...
    def __init__(
        ...
        self.component_style_checker = (StyleChecker(
            component_style_rgx, allow_acronyms) if component_style_rgx
                                        not in [None, ""] else StyleChecker(
                                            component_style, allow_acronyms))
        self.parameter_style_checker = (StyleChecker(
            parameter_style_rgx, allow_acronyms) if parameter_style_rgx
                                        not in [None, ""] else StyleChecker(
                                            parameter_style, allow_acronyms))
    ...
    def lint_single_file(self, file_path: str) -> int:
        ...
        num_errors = len(self.errors["components"]) + len(
    ...
    def check_parameter_names(self, data, errors: dict, parent_key: str = ""):
        ...
        for key, value in data.items():
            ...
            if isinstance(value, dict):
                self.check_parameter_names(value, errors,
                                           f"{parent_key}.{key}")
            elif (not self.parameter_style_checker.is_correct_style(key)
    ...
    def check_component_names(self, value, errors: dict, parent_key: str = ""):
        ...
        if component_name == "root":
            ...
            if not self.component_style_checker.is_correct_style(
                    component_name):
        ...
        for key, element in value.items():
            ...
            if isinstance(element, dict):
                if key in self.parameter_areas:
                    if not parent_key:
                        parent_key = "view"
                    self.check_parameter_names(element, errors,
                                               f"{parent_key}.{key}")
                else:
    ...
    def print_errors(self, file_path: str, errors: dict) -> None:
        ...
        if errors["components"]:
            ...
            error_logs.extend(
                [f"    - {error}" for error in errors["components"]])
        if errors["parameters"]:
            ...
            error_logs.extend(
                [f"    - {error}" for error in errors["parameters"]])
