...
class ViewModelBuilder:
	...
	def build_model(self, flattened_json: Dict[str, Any]) -> Dict[str, List[ViewNode]]:
		...
		for path, value in flattened_json.items():
			if '.binding.type' in path:
				...
				if binding_path not in binding_paths:
					...
					for transform_path, code in transforms:
						transform = TransformScript(transform_path, code, binding_path)
		...
		for path, value in flattened_json.items():
			if '.scripts.messageHandlers' in path and path.endswith('.messageType'):
				...
				handler = MessageHandler(base_path, script_code, message_type, scope)
		...
		for path, value in flattened_json.items():
			if '.scripts.customMethods' in path:
		...
		for path, value in flattened_json.items():
			...
			if '.events.' in path and '.config.script' in path:
				...
				if domain_type_match:
					...
					handler = EventHandlerScript(event_path, event_type, script_code, scope)
	...
	def _get_script_transforms(self, data: Dict[str, Any], binding_path: str) -> List[tuple]:
		...
		# Get transform code for each path
		for transform_path in transform_paths:
			code_path = f"{transform_path}.code"
			if code_path in data:
				transforms.append((transform_path, data[code_path]))
