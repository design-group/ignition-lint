...
from ignition_lint.common.flatten_json import flatten_json, read_json_file
...
def test_property_node_creation():
	...
	view_data_1 = {
		"custom": {
			"viewCustomProp": "custom value"
		},
		"params": {
			"viewParam": "param value"
		},
		"props": {
			"viewProp": "prop value"
		},
	...
	view_data_2 = {
		"root": {
			"children": [{
				...
				"custom": {
					"componentCustomProp": "custom value"
				},
				"props": {
					"text": "Button Text"
				}
	...
	for path, value in sorted(flattened.items()):
		...
		if not became_property:
			...
			if any(f".{skip}." in path for skip in ['meta', 'binding', 'scripts', 'events']):
				matching_skips = [
					skip for skip in ['meta', 'binding', 'scripts', 'events'] if f".{skip}." in path
				]
