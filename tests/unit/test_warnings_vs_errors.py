...
class TestWarningsVsErrorsInfrastructure(BaseRuleTest):
	...
	def test_name_pattern_rule_produces_warnings(self):
		...
		rule_config = get_test_config("NamePatternRule", 
		...
		self.assert_rule_summary(view_file, rule_config, "NamePatternRule", 
	...
	def test_name_pattern_rule_warning_content(self):
		...
		self.assert_rule_warnings(view_file, rule_config, "NamePatternRule", 
	...
	def test_polling_interval_rule_produces_errors(self):
		...
		self.assertEqual(rule_warnings, [], 
			f"PollingIntervalRule should not produce warnings, found: {rule_warnings}")
	...
	def test_mixed_rules_warnings_and_errors(self):
		...
		name_rule_config = get_test_config("NamePatternRule",
			target_node_types=["component"],
			convention="PascalCase",
			min_length=1
		)
		...
		self.assert_rule_summary(view_file, name_rule_config, "NamePatternRule",
			expected_warnings=1, expected_errors=0)
